...
run_exp() {
  ...
  if mpirun \
    ...
    "${PYTHON_ROOT}/torchrun" \
      ...
      "${script_path}" \
      --timestamp="${timestamp}" \
      --experiment-name="single_node_time_statistics" \
      --model='resnet18Cifar' \
      --dataset='cifar10' \
      --epochs "${epochs}" \
      --batch-size 256 \
...
run_exp "adamw" "none" 11
run_exp "adamw" "none" 12
#run_exp "adamw" "kfac" 38
#run_exp "adamw" "diag_kfac" 42
#run_exp "adafisher" "none" 66