...
class Trainer:
    ...
    def init(self, args, model):
        ...
        if model is None:
            self.model = create_image_classification_model(
                args.model, num_classes=self.data_manager.num_classes
            )
        else:
            self.model = model
        ...
        if args.model_ema:
            ...
            model_ema = utils.ExponentialMovingAverage(
                self.model_without_ddp, device=device, decay=1.0 - alpha
            )
    ...
    def init_logger(self, args):
        ...
        config_lines = []
        config_lines.append(
            f"Model Type           : {type(self.model).__name__ if not self.args.distributed else self.model_without_ddp.__class__.__name__}"
        )
        config_lines.append(
        ...
        config_lines.append(
            f"Scheduler            : {type(self.lr_scheduler).__name__}"
        )
        ...
        config_lines.append(
            f"EMA Model Used       : {'Yes' if self.model_ema else 'No'}"
        )
        config_lines.append(f"Output Directory     : {args.output_dir}")
        config_lines.append(
            f"Dataset Size         : {len(self.data_manager.train_loader.dataset)}"
        )
