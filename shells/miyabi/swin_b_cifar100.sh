...
run_exp() {
  ...
  if mpirun \
    ...
    "${PYTHON_ROOT}/torchrun" \
      ...
      "${script_path}" \
      --timestamp="${timestamp}" \
      --experiment-name="swin_b_multi_node" \
      --model='swin_b' \
      --dataset='cifar100' \
      --epochs "${epochs}" \
      --batch-size 256 \
      ...
      --kfac-factor-update-steps 12 \
      --kfac-inv-update-steps 128 \
      --kfac-damping 0.0028 \
      --kfac-kl-clip 0.0018 \
      --amp \
      ; then
...
run_exp "adamw" "kfac" 87
run_exp "adafisherw" "none" 109
run_exp "adamw" "diag_kfac" 113